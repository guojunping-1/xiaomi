<template>
  <div class="content" v-show="showChose">
      <section class="showChose" >
    <section class="address">
      <section class="title">
        <h4 style="font-size: 20px;margin-top: 26px; margin-left: 16px">居住地址</h4>
        <!--         <el-button @click="del" class="cancel" type="primary">取消</el-button> -->
        <span style="cursor: pointer;" @click="del">×</span>
      </section>
      <section class="title">
        <div class="area" style="font-size: 20px;margin-top: 16px;margin-left: 16px;" @click="provinceSelected()">{{Province?Province:info[province-1].name}}</div>
        <div class="area" style="font-size: 20px" @click="citySelected()" :class="City?'':'active'">{{City?City:'请选择'}}</div>
        <div class="area" style="font-size: 20px" @click="districtSelected()" :class="District?'':'active'" v-show="City">{{District?District:'请选择'}}</div>
      </section>
      <ul>
      ...
      <div class="bottom">
        <input type="text" class="detialAddress" v-if="isInputShow" v-model="home" placeholder="请输入地址">
        <input type="text" class="detialAddress" v-if="isInputShow" v-model="contact" placeholder="联系人">
        <input type="text" class="detialAddress" v-if="isInputShow" v-model="phoneNumber" placeholder="请输入手机号">
      </div>
        <div class="btn">
          <el-button :disabled="isDisabled" @click="close"  type="primary">确定</el-button>
        </div>
    </section>
  </section>
  </div>
...
export default {
  ...
  methods: {
    ...
    del() {
      ...
      this.district = "";
      this.$emit("change")
      this.$emit("detialAddress", this.home);
      this.$emit('getProvience', this.provience);
      this.$emit('getCity', this.city);
      this.$emit('getDistrict', this.district);
      this.$emit('getPhoneNumber', this.phoneNumber);
      this.$emit('getContact', this.contact);
    },
    choseAdd: function() {
      this.showChose = true;
    },
    // 给了数据关闭
    close: function() {
      this.$emit("detialAddress", this.input);
      this.$emit("change")
       this.phoneNumber = this.phoneNumber.substr(0, 3) + '****' + this.phoneNumber.substr(7);
      let place = { provience: this.provience, city: this.city, district: this.district, detial: this.home, phone: this.phoneNumber , contact : this.contact };
      this.addPlace(place);
    },
